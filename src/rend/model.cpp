...
Mesh Model::process_mesh(aiMesh* mesh, const aiScene* scene) {
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        if (material.diffuse_map == nullptr) {
            material.diffuse_map = glo::res->load_tex("tex_default_diffuse.png");
        }
        if (material.specular_map == nullptr) {
            material.specular_map = glo::res->load_tex("tex_default_specular.png");
        }
        if (material.emission_map == nullptr) {
            material.emission_map = glo::res->load_tex("tex_default_emission.png");
        }
        if (material.normal_map == nullptr) {
            material.normal_map = glo::res->load_tex("tex_default_normal.png");
        }
    }
...
std::vector<Texture*> load_material_textures(aiMaterial* mat, aiTextureType type) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        Texture* tex = glo::res->load_tex(std::string(str.C_Str()));
